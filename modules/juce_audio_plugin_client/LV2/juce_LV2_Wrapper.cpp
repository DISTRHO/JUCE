...
void findMaxTotalChannels (std::unique_ptr<AudioProcessor>& filter, int& maxTotalIns, int& maxTotalOuts)
{
...
#if JUCE_LINUX
bool JuceLv2UIWrapper::hostHasIdleInterface = false;
#endif
            ...
            else
            {
                ...
                for (i = 0; i < numOutChans; ++i)
                {
                    ...
                    if (i < numInChans && portAudioIns[i] != portAudioOuts[i])
                        FloatVectorOperations::copy (portAudioOuts[i], const_cast<const float*>(portAudioIns[i]), (int32) sampleCount);
                }
